...
def multiple_k(if_falldown=None, MODEL='live'):
    ...
    in_video_path = 'video-test/2.mp4'
    ...
    while video_cap.isOpened():
        ...
        action_name = ''
        if len(detection_target) > 0:
            for target_idx in range(len(detection_target)):

                box_l, box_t = detection_target[target_idx][1]
                box_r, box_b = detection_target[target_idx][2]
                ...
                if len(pose_landmarks) != 0:
                    for idx in range(len(pose_landmarks)):  # 实际上这里的idx只能是0
                        ...
                        video_pose_landmark[target_idx].append(pic_pose_landmark)

                        # 绘制关键点
                        output_frame = drawPoint(frame=output_frame, pts=pic_pose_landmark,
                                                 thinckness=PointThinness)  # 自定义绘制工具
                        ...
                        # action_name = 'fine'
                        # confid = 0
                        if len(video_pose_landmark[target_idx]) == ACTION_MODEL_MAX_FRAMES:
                            pts = np.array(video_pose_landmark[target_idx], dtype=np.float32)
        ...
        elif num_fall_down == 0 and key == 0: # 3.未跌倒
            FD.append(output_frame)

        elif num_fall_down == 0 and key != 0: # 4.跌倒过程结束
